...
INT h264_parse_sps(const BYTE *data, UINT dataSize, sps_info_struct *info)
{
    ...
    UINT index = 0;
    while (!data[index] || (data[index] == 0x01))
    {
        index++;
    }
    ...
    { // Nal SPS Flag
        ...
        if (info->profile_idc == 100 || info->profile_idc == 110 || info->profile_idc == 122 ||
            info->profile_idc == 244 || info->profile_idc == 44 || info->profile_idc == 83 || info->profile_idc == 86 ||
            info->profile_idc == 118 || info->profile_idc == 128 || info->profile_idc == 138 ||
            info->profile_idc == 139 || info->profile_idc == 134 || info->profile_idc == 135)
        {
        ...
        else if (pic_order_cnt_type == 1)
        {
            ...
            for (INT i = 0; (UINT)i < num_ref_frames_in_pic_order_cnt_cycle; i++)
            {
