...
RtpSendPs::RtpSendPs(const char *rtpServerIp, int rtpServerPort, int localRtpPort)
    : mRtpServerIp(rtpServerIp), mRtpServerPort(rtpServerPort), mLocalRtpPort(localRtpPort), mQuit(true)
{
...
int RtpSendPs::getNextNalu(FILE *inpf, unsigned char *buf)
{
    ...
    while (!startCodeFound)
    {
        ...
        // wh 没把 00 00 01分割符注释掉之前，视频呈慢放状态，视频帧率不够，也就是说时间戳打快了。把这里注释掉就正常了
        // if (info3 != 1)
    ...
    {
        ss << std::setw(2) << static_cast<unsigned>(buf[index]);
        std::cout << ss.str() << " ";
        ss.clear();
        ss.str("");
    }
    ...
    if (info2)
    {
        fseek(inpf, -3, SEEK_CUR); // fseek:重定位流(数据流/文件)上的文件内部位置指针
                                   // 返回值：成功，返回0，失败返回非0值，并设置error的值  SEEK_CUR	1
        return pos - 3;
    }
    if (info3)
    {
 ...
 *@return:   0 success, others failed
 */
int RtpSendPs::gb28181_make_pes_header(char *pData, int stream_id, int payload_len, unsigned long long pts,
...
int RtpSendPs::gb28181_send_rtp_pack(char *databuff, int nDataLen, int mark_flag, Data_Info_s *pPacker)
{
    ...
    {
        // 一帧数据发送完后，给mark标志位置1
        gb28181_make_rtp_header(szRtpHdr, ((mark_flag == 1) ? 1 : 0), ++pPacker->u16CSeq, pPacker->s64CurPts,
                                pPacker->u32Ssrc);
 ...
 * @return
 */
int RtpSendPs::gb28181_make_rtp_header(char *pData, int marker_flag, unsigned short cseq, long long curpts,
                                       unsigned int ssrc)
{
    bits_buffer_s bitsBuffer;
    if (pData == NULL)
...
// 发送数据包
int RtpSendPs::SendDataBuff(const char *buff, int size)
{
    /* 设置address */
    ...
    int ret = sendto(mSockFd, buff, size, 0, (struct sockaddr *)&addr_serv,
...
void RtpSendPs::SendDataThread(void *arg)
{
    ...
    Data_Info_s pPacker;
    pPacker.IFrame = 1;
    pPacker.u32Ssrc = (uint32_t)1234567890123; // 10进制的ssrc
    pPacker.s64CurPts = 0;
    // const char *filename = "../data/test-long.h264";
    // const char *filename = "../data/1.h264";
    const char *filename = "../data/195616.h264";
    ...
    {
        int size =
            rtpSendPs->getNextNalu(fp, (unsigned char *)(buf + PES_HDR_LEN)); // PES_HDR_LEN 19   size：发送数据的长度
        if (size <= 0)
        {
            LOGI("发送数据已完成，主动退出");
        ...
        // 判断读取出来的数据是否是分隔符
        if (rtpSendPs->isSpite(buf + PES_HDR_LEN))
        {
            memset((void *)(buf + PES_HDR_LEN), 0, 1024 * 1024 - 19);
            LOGI("continue\n");
            continue;
        }
...
void RtpSendPs::Send(const std::shared_ptr<Frame> &tFrame)
{
    if (tFrame->m_eMediaType == E_MEDIA_VIDEO)
    {
        // std::cout << "lenth: " << tFrame.TotalSize() << std::endl;
        sendPkt((char *)(tFrame->Data().get()), tFrame->TotalSize(), tFrame->m_pts, false);
        std::this_thread::sleep_for(std::chrono::milliseconds(40));
    }
    else if (tFrame->m_eMediaType == E_MEDIA_AUDIO)
    {
        // std::cout << "lenth: " << tFrame.TotalSize() << std::endl;
        sendPkt((char *)(tFrame->Data().get()), tFrame->TotalSize(), tFrame->m_pts, true);
    }
}

void RtpSendPs::Send(const uint8_t *buffer, size_t size, bool isVideo)
{
    SendDataBuff((const char *)buffer, size);
}

void RtpSendPs::sendPkt(char *buf, size_t size, uint64_t lPts, bool bIsAudio)
{
    ...
    {
        pPacker.IFrame = 1;
        pPacker.s64CurPts = lPts;
        // pPacker.s64CurPts += 3600;
    }
    else
    {
        pPacker.s64CurPts += 90;
    }

    gb28181_streampackageForH264(buf + PES_HDR_LEN, size - PES_HDR_LEN, &pPacker, bIsAudio);
}
